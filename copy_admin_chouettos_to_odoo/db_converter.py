...
Dump using:
mysqldump --compatible=postgresql --default-character-set=utf8 -r db.mysql -u root db
"""
...
def parse(input_filename, output_filename):
    ...
    if input_filename == "-":
        ...
        num_lines = int(
            subprocess.check_output(["wc", "-l", input_filename])
            .strip()
            .split()[0]
        )
    tables = {}
    ...
    for i, line in enumerate(input_fh):
        ...
        if current_table is None:
            ...
            # Inserting data into a table?
                ...
                output.write(
                    line.replace("'0000-00-00 00:00:00'", "NULL") + "\n"
                )
                num_inserts += 1
            # ???
            else:
        ...
        # Inside-create-statement handling
            ...
            if line.startswith('"'):
                ...
                try:
                    ...
                    extra = ""
                extra = re.sub(
                    r"CHARACTER SET [\w\d]+\s*",
                    "",
                    extra.replace("unsigned", ""),
                )
                extra = re.sub(
                    r"COLLATE [\w\d]+\s*", "", extra.replace("unsigned", "")
                )
                extra = re.sub(
                    r"COMMENT [\S]+\s*", "", extra.replace("unsigned", "")
                )
                ...
                if name == "id" and set_sequence is True:
                    sequence_lines.append(
                        "DROP SEQUENCE IF EXISTS %s_id_seq CASCADE"
                        % (current_table)
                    )
                    sequence_lines.append(
            ...
            # Is it the end of the table?
                ...
                output.write(
                    'DROP TABLE IF EXISTS "%s" CASCADE;' % current_table
                )
